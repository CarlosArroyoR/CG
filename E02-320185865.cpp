...
void CrearLetrasyFiguras()
{
	...
	//Cuadrado cafe(tronco) 5
	GLfloat vertices_cuadradocafe[] = {
		//X			Y			Z			R		G		B
		-0.5f,	-0.5f,		0.5f,			0.545f,	0.271f,	0.075f,
		0.5f,	-0.5f,		0.5f,			0.545f,	0.271f,	0.075f,
		0.5f,	0.5f,		0.5f,			0.545f,	0.271f,	0.075f,
		-0.5f,	-0.5f,		0.5f,			0.545f,	0.271f,	0.075f,
		0.5f,	0.5f,		0.5f,			0.545f,	0.271f,	0.075f,
...
int main()
{
   ...
   //Model: Matriz de Dimensi�n 4x4 en la cual se almacena la multiplicaci�n de las transformaciones geom�tricas.
	...
	while (!mainWindow.getShouldClose())
	{
		...
		meshColorList[3]->RenderMeshColor();//Recibir eventos del usuario
		glfwPollEvents();
		//Limpiar la ventana
		...
		model = glm::scale(model, glm::vec3(0.7f, 0.35f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se env�an al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		meshColorList[1]->RenderMeshColor();

		// Cuadrado rojo (cuerpo)
		model = glm::mat4(1.0);
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-0.27f, 0.0, -0.5f));
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, -0.8, -0.5f));
		model = glm::scale(model, glm::vec3(0.30f, 0.4f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se env�an al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		meshColorList[3]->RenderMeshColor();

		// Cuadrado cafe (tronco_arbol izq)
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-0.8f, -0.85, -1.0f));
		model = glm::scale(model, glm::vec3(0.2f, 0.35f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se env�an al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		meshColorList[5]->RenderMeshColor();

		// Cuadrado cafe (tronco_arbol der)
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.8f, -0.85, -1.0f));
		model = glm::scale(model, glm::vec3(0.2f, 0.35f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se env�an al shader como variables de tipo uniform
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-0.8f, -0.37, -1.0f));
