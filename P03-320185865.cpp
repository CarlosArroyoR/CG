...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Blanco M izquierdo
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.618f, -0.4f, -0.107f));
		model = glm::scale(model, glm::vec3(0.583f, 1.751f, 0.583f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		...
		color = glm::vec3(0.05f, 0.05f, 0.05f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[2]->RenderMeshGeometry(); //dibuja cubo y pirámide triangula
		...
		//Este
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(5.0f, 3.75f, 0.0f));
