...
float timeAcc = 0.0f;           // Acumulador de tiempo
const float scrollSpeed = 0.35f; // Velocidad visible del desplazamiento
GLfloat lastTime = 0.0f;
static double limitFPS = 1.0 / 60.0;


//  Estado para alternar decenas 1<->2
float tensAcc = 0.0f;            // acumulador de tiempo
const float tensEvery = 1.0f;    // cada cuántos segundos cambiar 
bool useNum1 = true;             // comienza mostrando el 1
...
void CreateObjects()
{
	...
	// --- Letrero (al final) ----
	unsigned int signIdx[] = { 0, 1, 2, 0, 2, 3 };
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		*/
		...
		// Avance del scroll del letrero (derecha -> izquierda)
		signAcc += deltaTime;
		float signU = fmodf(signAcc * signSpeed, 1.0f);


		tensAcc += deltaTime;
		if (tensAcc >= tensEvery) {
			tensAcc = 0.0f;
			useNum1 = !useNum1;  // alterna entre 1 y 2
		}
		...
		timeAcc += deltaTime; // acumula el tiempo

		// desplazamiento periódico entre 0 y 1
		float u = fmodf(timeAcc * scrollSpeed, 1.0f);
		glm::vec2 toffset(u, 0.0f);

		// activar shader y pasar uniform
		shaderList[0].UseShader();
		glUniform2fv(shaderList[0].getOffsetLocation(), 1, glm::value_ptr(toffset));
		...
		bool pressedG = mainWindow.getsKeys()[GLFW_KEY_G];
		...
		// Número cambiante 
		static int   numCol = 0;                 
		static float numSwitchAcc = 0.0f;        // acumulador
		const  float numSwitchEvery = 1.0f;      

		numSwitchAcc += deltaTime;
		if (numSwitchAcc >= numSwitchEvery) {
			numSwitchAcc = 0.0f;
			numCol = (numCol + 1) % 4;         
		}

		toffset = glm::vec2(numCol * 0.25f, 0.0f); 
		...
		if (useNum1) Numero1Texture.UseTexture();
